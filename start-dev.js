...
async function checkPrerequisites() {
    ...
    if (fs.existsSync('./ocean-backend/node_modules')) {
        ...
        logError('Backend dependencies not installed. Run "cd ocean-backend && npm install" first')
        allGood = false
    }
...
function startFrontend(useTurbopack = false) {
    ...
    frontend.stdout.on('data', (data) => {
        ...
        if (output.includes('Error') || output.includes('Warning') || output.includes('Ready') || output.includes('Local:')) {
    ...
    frontend.stderr.on('data', (data) => {
        ...
        if (!output.includes('Attention: Next.js now collects')) {
            process.stderr.write(`${colors.yellow}[Frontend Warning]${colors.reset} ${output}`)
        }
    })
